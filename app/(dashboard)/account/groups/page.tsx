...
export default function GroupsPage() {
  ...
  // Função para carregar grupos da organização
        ...
        const groupsWithMembers: Group[] = await Promise.all(
          groupsArray.map(async (apiGroup: ApiGroup) => {
            try {
              ...
              return {
                id: apiGroup.id,
                accountId: apiGroup.accountIds[0] || "",
                name: apiGroup.name,
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      ...
      try {
        const response = await listAccountMembers(selectedAccountId)
        if (response.success && response.data) {
                  ...
                  <div className="max-h-96 overflow-y-auto space-y-2 border rounded-lg p-2">
                    {availableMembers.length === 0 ? (
                      ...
                      availableMembers.map((user) => (
                        <div
                          key={user.id}
                          className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-colors ${selectedMembers.includes(user.id)
