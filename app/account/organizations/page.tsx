...
import { setCurrentAccountId, setDefaultAccountId, getCurrentAccountId, getDefaultAccountId } from "@/components/accounts/account-selector"
...
export default function ManageOrganizationsPage() {
  ...
  // Buscar grupos de uma organização
  const fetchGroups = async (accountId: string) => {
    // Evitar chamadas duplicadas
    if (isLoadingGroups[accountId]) {
      return
    }
    
    setIsLoadingGroups((prev) => ({ ...prev, [accountId]: true }))
    try {
      const response = await listGroups(accountId)
      if (response.success && response.data) {
        setGroups((prev) => ({ ...prev, [accountId]: response.data! }))
      } else {
        // Marcar como carregado mesmo em caso de erro para evitar loops
        setGroups((prev) => ({ ...prev, [accountId]: [] }))
        toast({
          title: "Error",
      ...
      setIsLoadingGroups((prev) => ({ ...prev, [accountId]: false }))
    }
  }
  ...
  // Carregar grupos quando expandir uma organização
  useEffect(() => {
  ...
  // Associar grupo existente à organização
  const handleAssociateGroup = async (groupId: string, accountId: string) => {
    setIsAssociatingGroup(true)
    try {
      const response = await addGroupToAccount(accountId, groupId)
      if (response.success) {
        toast({
          title: "Success",
  ...
  const handleDeleteGroup = async () => {
    ...
    const { groupId, accountId } = selectedGroupForRemoval
    try {
      const response = await removeGroupFromAccount(accountId, groupId)
      if (response.success) {
        toast({
          title: "Success",
  ...
  const fetchAccountMembers = async (accountId: string) => {
    setIsLoadingMembers((prev) => ({ ...prev, [accountId]: true }))
    try {
      const response = await listAccountMembers(accountId)
      if (response.success && response.data) {
        setAccountMembers((prev) => ({ ...prev, [accountId]: response.data! }))
      } else {
        toast({
          title: "Error",
      ...
      setIsLoadingMembers((prev) => ({ ...prev, [accountId]: false }))
    }
  }
    ...
    try {
      ...
      if (!userResponse.success || !userResponse.data) {
        ...
        const inviteData: CreateInviteData = {
          ...
          accountId: selectedAccountForMember.id,
          role: newMemberRole,
        }
      ...
      // Usuário encontrado - adicionar diretamente
      const response = await addAccountMember(selectedAccountForMember.id, {
        userId: userResponse.data.id,
        role: newMemberRole,
      })
      if (response.success) {
        ...
        setNewMemberEmail("")
        setNewMemberRole("member")
        const accountId = selectedAccountForMember.id
        setSelectedAccountForMember(null)
        await fetchAccountMembers(accountId)
      } else {
        toast({
          title: "Error",
  ...
  const handleRemoveMember = async (accountId: string, userId: string) => {
    ...
    try {
      const response = await removeAccountMember(accountId, userId)
      if (response.success) {
        toast({
          title: "Success",
  ...
  const handleUpdateMemberRole = async (
    ...
    try {
      const response = await updateAccountMemberRole(accountId, userId, { role })
      if (response.success) {
        toast({
          title: "Success",
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      if (response.success) {
        toast({
          title: "Success",
          description: `Organization "${selectedAccount.name}" deleted successfully.`,
        })
        setIsDeleteDialogOpen(false)
                                          ...
                                          >
                                            ...
                                            <div className="flex items-center gap-2">
                                              ...
                                              <Button
                                                ...
                                                size="sm"
                                                onClick={() =>
                                                  handleRemoveMember(account.id, member.userId)
                                                }
                                                disabled={
                                                    ...
                                                    {allGroups
                                                      ...
                                                      .map((group) => (
                                                        ...
                                                        >
                                                          ...
                                                          <Button
                                                            size="sm"
                                                            onClick={() => handleAssociateGroup(group.id, account.id)}
